...
internals.utility = {
    formatOutput(event, log, resource) {
        ...
        const entry = log.entry(resource, event.data);
        entry.severity = event.severity;
        entry.timestamp = {
            nanos: (timestamp % 1000) * 1000000,
            seconds: timestamp / 1000
        };
        entry.labels = event.tags.reduce((res, tag) => {
...
class GoodGcloud extends Stream.Writable {
    constructor(config) {
        ...
        this.gcloud = require('gcloud')(this._settings.gcloud);
        this.logging = this.gcloud.logging();
